                    ...
                    {(directoryTab === 'faculty' ? teachers : guards)
                      ...
                      <tr key={item.id} className={`transition-colors group hover:bg-slate-50`}>
                        ...
                        <td className="py-4 px-4 text-right opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <button 
                            onClick={() => setItemToDelete({ id: item.id, type: directoryTab, name: item.name })}
...
function EditGradeModal({ grade, onClose, onSave }: { grade: GradeSettings, onClose: () => void, onSave: (g: GradeSettings) => void }) {
  const [time, setTime] = useState(grade.dismissal_time);
  const [grace, setGrace] = useState(grade.grace_period_mins.toString());
...
function ConfirmDialog({ title, message, onConfirm, onCancel, variant = 'primary' }: { title: string, message: string, onConfirm: () => void, onCancel: () => void, variant?: 'primary' | 'danger' }) {
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      <motion.div 
        ...
        className="relative w-full max-w-sm bg-white rounded-2xl shadow-3xl p-8"
      >
        <div className={`w-12 h-12 ${variant === 'danger' ? 'bg-rose-100 text-rose-600' : 'bg-blue-100 text-blue-600'} rounded-xl flex items-center justify-center mb-6`}>
...
function StatCard({ label, value, delta, icon, color, bg }: { label: string, value: string, delta: string, icon: React.ReactNode, color: string, bg: string }) {
  return (
...
function LogEntry({ time, user, action, detail, type, status }: { time: string, user: string, action: string, detail: string, type: string, status?: 'error' }) {
  return (
